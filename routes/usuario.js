...
const usuarioTienePermiso = async (idUser, idRol) =>{
    ...
    try {
        ...
        if(user.recordsets[0].length) {
            console.log(`El usuario con ID ${idUser} YA cuenta con los permisos de tipo ${idRol}`);
            return true;
        } else {
            console.log(`Agregando los permisos ${idRol} al usuario con ID ${idUser}`);
            return false;
        }
    } catch (err) {
        console.error(err);
...
router.post("/", async (req, res) => {
    ...
    try{
        ...
        if (insert && insert.recordset && insert.recordset.length > 0) { // si si de dio de alta al usurio, tenemos que darle los permisos q dijo el admin
            console.log(insert.recordset[0].nuevoUsuarioId)
            
            if ( !(await usuarioTienePermiso(insert.recordset[0].nuevoUsuarioId,rol)) ){
